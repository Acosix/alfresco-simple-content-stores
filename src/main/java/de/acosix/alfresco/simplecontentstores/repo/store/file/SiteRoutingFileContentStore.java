    ...
    public void onMoveNode(final ChildAssociationRef oldChildAssocRef, final ChildAssociationRef newChildAssocRef)
    {
        ...
        if (StoreRef.STORE_REF_WORKSPACE_SPACESSTORE.equals(movedNode.getStoreRef()) && !EqualsHelper.nullSafeEquals(oldParent, newParent))
        {
            ...
            final Boolean moveRelevant = AuthenticationUtil.runAsSystem(new RunAsWork<Boolean>()
            {
                ...
                {
                    final NodeRef sourceSite = SiteRoutingFileContentStore.this.resolveSiteForNode(oldParent);
                    final NodeRef targetSite = SiteRoutingFileContentStore.this.resolveSiteForNode(newParent);

                    final SiteAwareFileContentStore sourceStore = SiteRoutingFileContentStore.this.resolveStoreForSite(sourceSite);
                    final SiteAwareFileContentStore targetStore = SiteRoutingFileContentStore.this.resolveStoreForSite(targetSite);
            ...
            final Map<NodeRef, NodeRef> copyMap)
    {
        ...
        if (StoreRef.STORE_REF_WORKSPACE_SPACESSTORE.equals(targetNodeRef.getStoreRef()))
        {
            ...
            final Boolean moveRelevant = AuthenticationUtil.runAsSystem(new RunAsWork<Boolean>()
            {
                ...
                public Boolean doWork()
                {
                    final NodeRef sourceSite = SiteRoutingFileContentStore.this.resolveSiteForNode(sourceNodeRef);
                    final NodeRef targetSite = SiteRoutingFileContentStore.this.resolveSiteForNode(targetNodeRef);
